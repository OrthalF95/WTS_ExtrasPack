...
default dap_map_location = "room_s"
...
label set_dap_map_location(location = ""):
    #ast_random_number (1-5), gets defined once during the day and once during the nigh.
    if location != "":
        if location == "courtyard":
            $ dap_map_location = "courtyard"
        elif location in ["slytherin_room","slyth_room","room_s"]:
            $ dap_map_location = "room_s"
        elif location in ["defense_classroom"]:
            $ dap_map_location = "defense"

    else: #Random
        if dap_random_number in [1,2]:
            $ dap_map_location = "courtyard"
        else: #Slytherin Room
            $ dap_map_location = "room_s"
...
label update_character_map_locations:
    ...
    #Daphne
    if dap_map_location == "room_s":
        $ dap_map_xpos = 476
        $ dap_map_ypos = 118
    if dap_map_location == "courtyard":
        $ dap_map_xpos = 634
        $ dap_map_ypos = 254
    if dap_map_location == "defense": #Event
        $ dap_map_xpos = 530
        $ dap_map_ypos = 190
...
screen map_screen_characters():
    ...
    if daphne_unlocked:
        imagebutton:
            xpos UI_xpos_offset +dap_map_xpos
            ypos UI_ypos_offset +dap_map_ypos
            xalign 0.5
            yalign 0.5
            idle "interface/map/name_daphne.webp"
            hover "interface/map/name_daphne_hover.webp"
            hovered SetVariable("ball_hint", "summon_daphne")
            unhovered SetVariable("ball_hint", None)
            action Return("daphne")
