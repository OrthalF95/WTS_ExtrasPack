...
label update_stats:
    ...
    # Whoring
    $ dap_whoring_word_list = ["Pure", "Naive", "Curious", "Naughty", "Perverse", "Immoral", "Slutty", "Shameless", "Cumslut", "Total Cumslut", "Shameless Cumslut"]
    $ dap_whoring_word = dap_whoring_word_list[int(dap_whoring/2.4)]

    # Reputation
    #$ dap_reputation_word_list = ["Tomboy", "Team Player", "Quidditch Star", "Flying Ace", "Minx", "Manipulative", "Exploiting", "Cheater", "Team's Cumdump", "Quidditch Whore", "Cheating Slut"]
    #$ dap_reputation_word = dap_reputation_word_list[int(dap_reputation/2.4)]

    # Mood
    $ dap_mood_word_list = ["Cheerful", "Reluctant", "Gloomy", "Stern", "Slightly Annoyed", "Annoyed", "Upset", "Outraged", "Mad", "Angry", "Very Angry"]
    if dap_mood >= 0 and dap_mood <= 10:
        $ dap_mood_word = dap_mood_word_list[int(dap_mood/1.0)]
    else:
        $ dap_mood_word = "Very Angry"
...
label stats_menu(xx=150, yy=90):
    ...
    $ stats_dict = {
                    ...
                    "Daphne": {"ico": "daphne", "flag": daphne_unlocked, "name": "Daphne Greengrass", "sex": "Female", "height": "5.1ft", "weight": "135lb", "job": "Student", "hates": "You", "likes": "Drago {size=-4}Secretly..{/size}"}
                    }

    $ stats_categories_sorted = ["Genie", "Snape", "Tonks", "Hermione", "Cho", "Luna", "Astoria", "Susan", "Daphne"] #"Ginny", "Daphne", "Padma", "Patil", "Myrtle", "Mafkin"
                    ...
                    if current_item["flag"]:
                        vbox:
                            ...
                            if current_category == "Genie":
                                ...
                                use stat_bar(int(10-dap_mood/1.0), "-Mood-" , dap_mood_word, dap_mood)
                                use stat_bar(int(dap_tier/0.3), "-Favour Tier-", "", dap_tier) # 4 will be max.
                                use stat_bar(int(dap_whoring/0.9), "-Whoring-", dap_whoring_word, dap_whoring)
                                #use stat_bar(int(dap_reputation/0.9), "-Reputation-", dap_reputation_word, dap_reputation)
