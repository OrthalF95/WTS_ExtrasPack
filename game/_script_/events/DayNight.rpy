...
label update_day_values:
    ...
    if dg_event_pause > 0:
        $ dg_event_pause -= 1
    if dg_summon_pause > 0:
        $ dg_summon_pause -= 1
    $ gave_daphne_gift  = False
...
label update_day_and_night_values:
    ...
    if dg_summon_pause == 0:
        $ daphne_busy = False
    $ chitchated_with_daphne = False
    $ dap_random_number = renpy.random.randint(1, 5) #Used for Map screen. Gets defined once during day and night.
    call set_dap_map_location()
